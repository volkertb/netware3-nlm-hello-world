...
- Functional correctness still requires booting `floppy.img` on real or emulated NetWare
  3.11/3.12 — a manual step until the QEMU sidecar exists. Last full boot verification:
  2026-07-19, both NLMs work, including the graphics switch that used to abend. Root cause of
...
1. QEMU sidecar ([docs/qemu-vm-debugging.md](docs/qemu-vm-debugging.md)) — NetWare VM with VNC
   for the human and a QMP control channel for the agent, replacing the manual build-boot-reset
   cycle with a fast joint human+agent loop.
