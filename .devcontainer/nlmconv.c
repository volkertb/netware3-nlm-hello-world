...
main (int argc, char **argv)
{
  ...
  *nlm_extended_header (outbfd) = extended_hdr_struct;
  ...
  if (custom_file != NULL)
	...
	{
	  fprintf (stderr, "%s:%s: %s (error while adding sections for header info)\n", program_name, custom_file,
		   strerror (errno));
	  custom_file = NULL;
	}
      else
  ...
  if (help_file != NULL)
	...
	{
	  fprintf (stderr, "%s:%s: %s (opening help file)\n", program_name, help_file,
		   strerror (errno));
	  help_file = NULL;
	}
      else
  ...
  if (message_file != NULL)
	...
	{
	  fprintf (stderr, "%s:%s: %s (error while opening message file)\n", program_name, message_file,
		   strerror (errno));
	  message_file = NULL;
	}
      else
  ...
  if (rpc_file != NULL)
	...
	{
	  fprintf (stderr, "%s:%s: %s (error while opening RPC file)\n", program_name, rpc_file,
		   strerror (errno));
	  rpc_file = NULL;
	}
      else
  ...
  if (sharelib_file != NULL)
	...
	{
	  fprintf (stderr, "%s:%s: %s (error while opening or checking format of sharelib file)\n", program_name, sharelib_file,
      ...
      else
	{
	  ...
	  if (shared_data == NULL
	    ...
	    {
	      fprintf (stderr, "%s:%s: %s (error while opening shared data)\n", program_name, sharelib_file,
		       strerror (errno));
	      sharelib_file = NULL;
	    }
	  else
...
link_inputs (struct string_list *inputs, char *ld, char * mfile)
{
  ...
  argv = (char **) xmalloc ((c + 8) * sizeof (char *));
